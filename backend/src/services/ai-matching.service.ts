...
function calculateSkillMatch(projectSkills: string[], talentSkills: string[]): number {
  if (projectSkills.length === 0) return 50; // Neutral if no skills specified
  
  const matchingSkills = projectSkills.filter(skill => 
    talentSkills.some(ts => ts.toLowerCase() === skill.toLowerCase())
  );
...
function calculateBudgetMatch(
  ...
  talentHourlyRate: number
): number {
  if (!projectBudgetMin && !projectBudgetMax) return 50;
...
function calculateExperienceMatch(
  ...
  const levelRequirements: Record<string, { minYears: number; minProjects: number }> = {
  ...
  if (meetsYears && meetsProjects) return 100;
  if (meetsYears || meetsProjects) return 70;
...
export async function findMatchingTalent(
  project: Project,
  options: {
    limit?: number;
    minScore?: number;
  ...
  for (const talent of talents) {
    const skillNames = talent.skills.map((s: any) => s.skill.name);
    ...
    const reputationScore = calculateReputationScore(
      ...
      talent.completedProjects,
      Number(talent.totalEarnings) || 0
    );

    // Weighted total score
    const totalScore = Math.round(
      skillScore * 0.35 +        // 35% - Skills match
    ...
    if (totalScore >= minScore) {
      ...
      if (skillScore >= 60 && skillScore < 80) reasons.push('Good skill match');
      if (budgetScore === 100) reasons.push('Within budget');
      if (experienceScore === 100) reasons.push('Experience level matched');
      if (reputationScore >= 70) reasons.push('Highly rated');
      if (talent.completedProjects >= 10) reasons.push('Proven track record');
...
export async function findMatchingProjects(
  ...
  for (const project of projects) {
    const projectSkillNames = project.skills.map((s: any) => s.skill.name);
    
    const skillScore = calculateSkillMatch(projectSkillNames, skillNames);
    const budgetScore = calculateBudgetMatch(
      project.budgetMin ? Number(project.budgetMin) : undefined,
      project.budgetMax ? Number(project.budgetMax) : undefined,
      talent.hourlyRate || 0
    );
    const experienceScore = calculateExperienceMatch(
      project.experienceLevel,
    ...
    const totalScore = Math.round(
      skillScore * 0.50 +        // 50% - Skills (higher for talent view)
    ...
    if (totalScore >= minScore) {
      ...
      if (skillScore >= 80) reasons.push('Skills align perfectly');
      if (budgetScore === 100) reasons.push('Rate fits budget');
      if (project.client?.verified) reasons.push('Verified client');
      if (project.urgent) reasons.push('Urgent project');
...
export async function autoInviteTalent(
  projectId: string,
  maxInvites: number = 5
): Promise<number> {
